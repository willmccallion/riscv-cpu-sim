...
pub fn decode_stage(cpu: &mut Cpu) -> Result<(), String> {
    ...
    match d.opcode {
        ...
        opcodes::OP_LOAD => {
            ...
            c.mem_read = true;
            c.alu = AluOp::Add;
            (c.width, c.signed_load) = match d.funct3 {
        ...
        opcodes::OP_SYSTEM => {
            ...
            match d.raw {
                ...
                _ => {
                    c.csr_addr = d.raw >> 20;
                    c.a_src = OpASrc::Reg1; // RS1 source (or zimm for *I)
                    c.b_src = OpBSrc::Zero;
                    ...
                    c.reg_write = d.rd != 0; // rd gets old CSR unless rd==0
                }
            }
        }
        opcodes::OP_IMM_32 | opcodes::OP_REG_32 => {
            c.reg_write = true;
            c.is_rv32 = true;
        }
